...
async function initCarousel() {
    ...
    try {
        ...
        if (carouselInner && indicators) {
            carouselInner.innerHTML = products.map((product, index) => `
                <div class="carousel-item ${index === 0 ? 'active' : ''}">
                    <img src="${product.image}" class="d-block w-100 d-img" alt="${product.title}" loading="lazy">
...
async function displayProducts() {
    ...
    const fetchCategoryPromises = categories.map(async (category) => {
        ...
        try {
            const response = await fetch(`https://fakestoreapi.com/products/category/${category.apiName}`);
            if (!response.ok) throw new Error('Network response was not ok');
            const products = await response.json();
            ...
            container.innerHTML = products.map(product => `
                <div class="product-card col-md-3">
                    ...
                    <div class="product-rating p-3">Rating: ${generateStars(product.rating.rate)}</div>
                    <a class="btn btn-custom" href="producto.html?id=${product.id}">Ver Producto</a>
                </div>
            `).join('');

        } catch (error) {
            console.log(`Error fetching products for category ${category.apiName}:`, error);
        }
    });
...
async function displayProductDetails() {
    ...
    if (!productImage || !productTitle || !productDescription || !productPrice || !buyButton) return;
    ...
    try {
        ...
        buyButton.addEventListener('click', () => {
            ...
            console.log(`Producto: ${product.title}, cantidad seleccionada: ${quantity}`);
        });
